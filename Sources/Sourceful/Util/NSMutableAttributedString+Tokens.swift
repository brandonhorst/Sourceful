	...
	convenience init(source: String, tokens: [Token], theme: SyntaxColorTheme) {
		...
		for token in tokens {
			...
			if token.isEditorPlaceholder {
				...
				self.addAttributes([.foregroundColor: SourcefulColor.clear, .font: SourcefulFont.systemFont(ofSize: 0.01)], range: startRange)
				self.addAttributes([.foregroundColor: SourcefulColor.clear, .font: SourcefulFont.systemFont(ofSize: 0.01)], range: endRange)
